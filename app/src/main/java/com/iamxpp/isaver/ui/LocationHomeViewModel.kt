...
class LocationHomeViewModel(private val resolver:LocationHomeAppResolver,private val store:LocationHomeCustomStore,private val fs:RootFileSystem,private val dispatcher:CoroutineDispatcher):ViewModel(){
 ...
 fun removeCustomLocation(id:LocationId){val g=++operationGeneration;mutable.value=mutable.value.copy(operationInProgress=true,addError=null);viewModelScope.launch{try{val result=store.remove(id);if(g==operationGeneration)finish(mapResult(result),g)}catch(c:CancellationException){throw c}catch(_:Exception){if(g==operationGeneration)finish("删除位置失败",g)}}}
 private fun mutate(name:String,rawPath:String,id:LocationId?){val g=++operationGeneration;viewModelScope.launch{mutable.value=mutable.value.copy(operationInProgress=true,addError=null);val n=name.trim();if(n.isEmpty()){finish("名称不能为空",g);return@launch};val p=RootPath.parse(rawPath).getOrElse{finish("路径格式无效",g);return@launch};try{when(val s=withContext(dispatcher){fs.stat(p)}){is OperationResult.Failure->{finish(mapCode(s.code),g);return@launch};is OperationResult.Success->{if(s.value.type!=EntryType.DIRECTORY){finish("路径不是目录",g);return@launch};if(!s.value.readable){finish("目录不可读",g);return@launch};if(!s.value.writable){finish("目录不可写",g);return@launch}}};finish(mapResult(if(id==null)store.add(n,p)else store.update(id,n,p)),g)}catch(c:CancellationException){throw c}catch(_:Exception){finish("保存位置失败",g)}}}
 private fun finish(e:String?,g:Long){if(g==operationGeneration)mutable.value=mutable.value.copy(operationInProgress=false,addError=e)}
