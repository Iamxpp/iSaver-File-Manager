...
class LibsuRootFileSystem internal constructor(
    ...
    override suspend fun createDirectory(parent:RootPath,name:FolderName):OperationResult<DirectoryEntry>{
        ...
        if(p.type!=com.isaver.filemanager.domain.EntryType.DIRECTORY)return failure(ErrorCode.NOT_DIRECTORY,"路径不是目录","Parent was not directory")
        ...
        if(finalChild.value.type!=com.isaver.filemanager.domain.EntryType.DIRECTORY||finalChild.value.symbolicLink)return uncertain("Created child was not a plain directory")
        return finalChild
    }
    ...
    override suspend fun createFileNoReplace(
        ...
        if (
            ...
            created.value.type != com.isaver.filemanager.domain.EntryType.FILE ||
    ...
    override suspend fun copyToOutput(
        ...
        if (sourceEntry.value.type != com.isaver.filemanager.domain.EntryType.FILE ||
    ...
    override suspend fun replaceFileAtomically(
        ...
        if (EntryName.join(sourceDirectory, sourceName) != source.path ||
            source.type != com.isaver.filemanager.domain.EntryType.FILE || source.symbolicLink ||
        ...
        if (finalEntry !is OperationResult.Success || finalIdentity !is OperationResult.Success ||
            finalEntry.value.type != com.isaver.filemanager.domain.EntryType.FILE || finalEntry.value.symbolicLink ||
    ...
    override suspend fun changeMode(
        ...
        if (mode !in 0..0x1FF || expectedMetadata.mode !in 0..0x1FF ||
            ...
            source.type == com.isaver.filemanager.domain.EntryType.OTHER
        ) {
    ...
    override suspend fun commitExtractionStage(
        ...
        return withContext(NonCancellable) {
            ...
            if (entry !is OperationResult.Success ||
                entry.value.type != com.isaver.filemanager.domain.EntryType.DIRECTORY ||
    ...
    private suspend fun transfer(
        ...
        return withContext(NonCancellable){
            ...
            if(finalEntry.value.type!=com.isaver.filemanager.domain.EntryType.FILE||finalEntry.value.symbolicLink||finalEntry.value.sizeBytes!=expectedSizeBytes){
    ...
    override suspend fun moveFileAsNoReplace(
        ...
        if (
            source.type != com.isaver.filemanager.domain.EntryType.FILE ||
            source.symbolicLink ||
            !source.readable ||
        ...
        if (
            ...
            currentSource.value.type != com.isaver.filemanager.domain.EntryType.FILE ||
        ...
        if (
            ...
            targetAfter.value.type != com.isaver.filemanager.domain.EntryType.FILE ||
    ...
    override suspend fun deleteEntryPermanently(
        ...
        if (source.type == com.isaver.filemanager.domain.EntryType.OTHER || source.symbolicLink ||
    ...
    private suspend fun directoryOperation(
        ...
        if (
            source.type != com.isaver.filemanager.domain.EntryType.DIRECTORY || source.symbolicLink ||
        ...
        if (
            currentSource !is OperationResult.Success ||
            currentSource.value.type != com.isaver.filemanager.domain.EntryType.DIRECTORY ||
    ...
    private suspend fun reconcileDirectoryResult(
        ...
        val sourceMatches = sourceAfter is OperationResult.Success &&
            sourceAfter.value.type == com.isaver.filemanager.domain.EntryType.DIRECTORY &&
        ...
        if (
            ...
            target.value.type != com.isaver.filemanager.domain.EntryType.DIRECTORY || target.value.symbolicLink ||
    ...
    private suspend fun prepareDirectory(
        ...
        if(first.value.type!=com.isaver.filemanager.domain.EntryType.DIRECTORY)return failure(ErrorCode.NOT_DIRECTORY,"路径不是目录","Parent was not directory")
        ...
        if(canonicalStat.value.symbolicLink||canonicalStat.value.type!=com.isaver.filemanager.domain.EntryType.DIRECTORY)return failure(ErrorCode.COMMAND_FAILED,"${role}无效","Canonical parent was not a plain directory")
