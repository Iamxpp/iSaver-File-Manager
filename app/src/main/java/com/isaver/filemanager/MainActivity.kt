...
import com.isaver.filemanager.export.ExternalOpenIntentFactory
import com.isaver.filemanager.export.ExternalShareIntentFactory
import kotlinx.coroutines.CoroutineDispatcher
import com.isaver.filemanager.ui.RootGateScreen
import com.isaver.filemanager.ui.RootGateViewModel
import com.isaver.filemanager.ui.RootGateUiState
import com.isaver.filemanager.ui.BrowserViewModel
import com.isaver.filemanager.ui.HomeBackResult
import com.isaver.filemanager.ui.HomeDestination
import com.isaver.filemanager.ui.ISaverHomeScreen
import com.isaver.filemanager.ui.ISaverHomeViewModel
...
import com.isaver.filemanager.ui.canUseRealTarget
import com.isaver.filemanager.ui.files.HomeTab
import com.isaver.filemanager.ui.theme.ISaverTheme
import com.isaver.filemanager.ui.archive.ArchiveBackResult
import com.isaver.filemanager.ui.archive.ArchiveViewModel
import com.isaver.filemanager.ui.recent.RecentOpenTarget
import com.isaver.filemanager.ui.recent.RecentViewModel
import com.isaver.filemanager.share.ShareSourceLocationResolver
import com.isaver.filemanager.transfer.TransferUiState
import com.isaver.filemanager.transfer.TransferViewModel
import com.isaver.filemanager.preview.RootPreviewRepository
import com.isaver.filemanager.ui.virtualviews.VirtualViewRepositoryStore
import com.isaver.filemanager.ui.virtualviews.VirtualViewViewModel
import com.isaver.filemanager.ui.dualpane.DualPaneBrowserCallbacks
import com.isaver.filemanager.ui.dualpane.DualPaneViewModel
import com.isaver.filemanager.ui.dualpane.PaneId
import com.isaver.filemanager.ui.dualpane.other
import com.isaver.filemanager.ui.device.DeviceOverviewRepository
import com.isaver.filemanager.ui.device.DeviceSettingsScreen
import com.isaver.filemanager.ui.device.DeviceSettingsViewModel
import com.isaver.filemanager.data.access.FileAccessMode
import com.isaver.filemanager.texteditor.TextEditorScreen
import com.isaver.filemanager.texteditor.TextEditorViewModel
import com.isaver.filemanager.filetools.FileToolsScreen
import com.isaver.filemanager.filetools.FileToolsViewModel
        ...
        setContent {
            ...
            ISaverTheme {
                if (
                    ...
                    var previousAccessMode by remember { mutableStateOf<FileAccessMode?>(null) }
                    ...
                    LaunchedEffect(accessMode) {
                        val previous = previousAccessMode
                        previousAccessMode = accessMode
                        if (previous == null || previous == accessMode) return@LaunchedEffect
                    ...
                    LaunchedEffect(archiveState.operation) {
                        val success = archiveState.operation as? com.isaver.filemanager.archive.ArchiveState.Success
                            ?: return@LaunchedEffect
                    ...
                    fun handleRecentOpen(item: com.isaver.filemanager.ui.recent.RecentUiItem) {
...
internal class TextEditorViewModelFactory(
    private val repository: com.isaver.filemanager.texteditor.TextEditorRepository,
    private val drafts: com.isaver.filemanager.texteditor.TextDraftStore,
...
internal class FileToolsViewModelFactory(
    private val hexRepository: com.isaver.filemanager.filetools.HexViewerRepository,
    private val comparisonRepository: com.isaver.filemanager.filetools.FileComparisonRepository,
) : ViewModelProvider.Factory {
    override fun <T : ViewModel> create(modelClass: Class<T>): T {
...
internal class BrowserViewModelFactory(
    ...
    private val archiveRepository: com.isaver.filemanager.archive.ArchiveRepository? = null,
    private val recentRepository: com.isaver.filemanager.recent.RecentRepository? = null,
    private val ioDispatcher: CoroutineDispatcher = Dispatchers.IO,
    private val rootExportRepository: com.isaver.filemanager.export.RootExportRepository? = null,
    private val directoryShareRepository: com.isaver.filemanager.archive.DirectoryShareRepository? = null,
    private val fileMoveRepository: com.isaver.filemanager.fileops.FileMoveRepository? = null,
    private val fileCopyRepository: com.isaver.filemanager.fileops.FileCopyRepository? = null,
    private val fileRenameRepository: com.isaver.filemanager.fileops.FileRenameRepository? = null,
    private val operationTaskStore: com.isaver.filemanager.tasks.OperationTaskStore? = null,
    private val trashRepository: com.isaver.filemanager.trash.TrashRepository? = null,
    private val bookmarkRepository: com.isaver.filemanager.bookmarks.BookmarkRepository? = null,
    private val virtualViewRepository: com.isaver.filemanager.virtualviews.VirtualViewRepository? = null,
    private val browserSessionStore: BrowserSessionStore? = null,
    private val previewRepository: RootPreviewRepository? = null,
    private val checksumFile: suspend (com.isaver.filemanager.domain.DirectoryEntry) -> OperationResult<String> = {
        OperationResult.Failure(com.isaver.filemanager.domain.ErrorCode.COMMAND_FAILED, "无法计算校验和")
    },
    private val checksumFileByAlgorithm: suspend (com.isaver.filemanager.domain.DirectoryEntry, com.isaver.filemanager.fileops.ChecksumAlgorithm) -> OperationResult<String> = { entry, algorithm ->
        if (algorithm == com.isaver.filemanager.fileops.ChecksumAlgorithm.SHA256) checksumFile(entry)
        else OperationResult.Failure(com.isaver.filemanager.domain.ErrorCode.COMMAND_FAILED, "不支持此校验算法")
    },
) : ViewModelProvider.Factory {
    override fun <T : ViewModel> create(modelClass: Class<T>): T {
        ...
        return BrowserViewModel(
            ...
            recordDirectoryAccess = { path, title ->
                recentRepository?.recordAccess(
                    ...
                    title,
                    null,
                    com.isaver.filemanager.recent.RecentItemType.DIRECTORY,
                )
            },
            recordFileAccess = { path, title ->
                recentRepository?.recordAccess(
                    ...
                    title,
                    null,
                    com.isaver.filemanager.recent.RecentItemType.FILE,
                )
            },
            exportFile = rootExportRepository?.let { repository -> repository::export } ?: { entry ->
                OperationResult.Failure(com.isaver.filemanager.domain.ErrorCode.COMMAND_FAILED, "无法打开文件")
            },
            shareFile = rootExportRepository?.let { repository -> repository::share } ?: { entry ->
                OperationResult.Failure(com.isaver.filemanager.domain.ErrorCode.COMMAND_FAILED, "无法分享文件")
            },
            ...
            moveFile = fileMoveRepository?.let { repository ->
                ...
                OperationResult.Failure(com.isaver.filemanager.domain.ErrorCode.COMMAND_FAILED, "无法移动文件")
            },
            copyFile = fileCopyRepository?.let { repository ->
                ...
                OperationResult.Failure(com.isaver.filemanager.domain.ErrorCode.COMMAND_FAILED, "无法复制文件")
            },
            renameFile = fileRenameRepository?.let { repository -> repository::rename } ?: { _, _, _ ->
                OperationResult.Failure(com.isaver.filemanager.domain.ErrorCode.COMMAND_FAILED, "无法重命名文件")
            },
...
internal class RecentViewModelFactory(
    private val repository: com.isaver.filemanager.recent.RecentRepository,
    private val fileSystem: RootFileSystem,
    private val ioDispatcher: CoroutineDispatcher = Dispatchers.IO,
) : ViewModelProvider.Factory {
    override fun <T : ViewModel> create(modelClass: Class<T>): T {
...
internal class ArchiveViewModelFactory(
    private val repository: com.isaver.filemanager.archive.ArchiveRepository,
    private val recentRepository: com.isaver.filemanager.recent.RecentRepository,
    private val operationTaskStore: com.isaver.filemanager.tasks.OperationTaskStore? = null,
    private val ioDispatcher: CoroutineDispatcher = Dispatchers.IO,
) : ViewModelProvider.Factory {
    override fun <T : ViewModel> create(modelClass: Class<T>): T {
        ...
        return ArchiveViewModel(
            ...
            recordAccess = { path, title ->
                recentRepository.recordAccess(
                    ...
                    title,
                    null,
                    com.isaver.filemanager.recent.RecentItemType.ARCHIVE,
                )
            },
...
internal class VirtualViewViewModelFactory(
    private val store: com.isaver.filemanager.ui.virtualviews.VirtualViewStore,
...
private class RootGateViewModelFactory(
    ...
    private val modeStore: com.isaver.filemanager.data.access.FileAccessModeStore,
    private val accessController: com.isaver.filemanager.data.access.FileAccessController,
) : ViewModelProvider.Factory {
    override fun <T : ViewModel> create(modelClass: Class<T>): T {
