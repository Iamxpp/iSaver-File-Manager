...
class BatchRenameTest {
    ...
    fun `executor uses temporary names and handles swaps`() = runTest {
        ...
        val plan = BatchRenamePlan(listOf(
            BatchRenameItem(current.getValue("a"), com.isaver.filemanager.domain.EntryName.parse("b").getOrThrow()),
            BatchRenameItem(current.getValue("b"), com.isaver.filemanager.domain.EntryName.parse("a").getOrThrow()),
        ))
    ...
    fun `executor preserves preview order when a planned item is unchanged`() = runTest {
        ...
        val plan = BatchRenamePlan(listOf(item("change", "changed"), BatchRenameItem(
            unchanged,
            com.isaver.filemanager.domain.EntryName.parse("keep").getOrThrow(),
        )))
    ...
    private fun item(source: String, target: String) = BatchRenameItem(
        entry(source), com.isaver.filemanager.domain.EntryName.parse(target).getOrThrow(),
    )
