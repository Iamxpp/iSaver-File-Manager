...
class ArchiveRootInstrumentedTest {
    ...
    fun rootZipCreateInspectAndExtractLeavesNoArchiveStages() = runBlocking {
        ...
        try {
            ...
            val snapshot = (app.rootFileSystem.readDirectory(source) as com.isaver.filemanager.domain.OperationResult.Success).value
            ...
            assertTrue("ZIP inspect failed: $listing", listing is com.isaver.filemanager.domain.OperationResult.Success)
            listing as com.isaver.filemanager.domain.OperationResult.Success
            ...
            listOf(
                ...
                assertTrue("$format created archive inspect failed: $inspected", inspected is com.isaver.filemanager.domain.OperationResult.Success)
                inspected as com.isaver.filemanager.domain.OperationResult.Success
            ...
            expectedFormats.forEach { (name, format) ->
                ...
                assertTrue("$format inspect failed: $inspected", inspected is com.isaver.filemanager.domain.OperationResult.Success)
                inspected as com.isaver.filemanager.domain.OperationResult.Success
                assertEquals(format, (inspected.value as ArchiveListing).format)
    ...
    private fun testArchiveRepository(app: ISaverApplication) = ArchiveRepository(
        ...
        issueSource = { cached ->
            app.incomingStreamRegistry.issue(cached).fold(
                onSuccess = { com.isaver.filemanager.domain.OperationResult.Success(it) },
                onFailure = {
                    com.isaver.filemanager.domain.OperationResult.Failure(
                        com.isaver.filemanager.domain.ErrorCode.SOURCE_UNREADABLE,
                        "无法读取测试夹具",
                    )
                },
            )
