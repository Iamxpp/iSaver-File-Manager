...
> 2026-08-14 命名空间与最近页性能同步：预发布 Android `namespace`、`applicationId`、主/测试/Debug/remote 源码包、JNI 符号、Provider authority 和脚本统一迁移为 `com.isaver.filemanager`，旧包不提供兼容壳。最近项目校验改为事件驱动：Room 只在 `available` 真正变化时更新；Repository 将可用性视为缓存而非列表内容变化，阻断 `stat -> UPDATE -> Flow -> stat` 反馈环；刷新复用已解析条目，初始 `accessMode` 不再额外触发全局刷新。小米 9 最近页 10 秒空闲 `/proc` CPU tick 增量为 0，5 秒 `gfxinfo` 无持续帧；668 个 JVM 测试、Lint、全构建及 13 个本轮真机专项通过。审计见 `docs/audits/2026-08-14-namespace-performance-audit.md`。
